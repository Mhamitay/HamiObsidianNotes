...
> Pulls completed tasks, work items, lessons, and #win lines from your daily notes for this month.
...
if (wins.length) {
  dv.header(3, "🏆 Wins");
  for (const w of wins.slice(0, 60)) dv.listItem(`**${w.d}** — ${w.t} <br><small>${w.l}</small>`);
}
